                  ...
                  <td class="text-left"><a href="<?php echo $product['href']; ?>"><?php echo $product['name']; ?></a>
					...
					<?php if ( isset($product['design']) && $product['design'] != false) { $design = $product['design'];?>
						...
						if (isset($design->options) && $design->options != '[]')
						{
							...
							{							
								if (count($design_options[$i]->value) > 0)
								{
							...
							{								
								...
								foreach($images as $view => $src)
								{
									$img .= '<div class="img-thumbnail" style="margin:2px;">'
											...
											.	'<br /><a target="_blank" href="'.$base_url.'tshirtecommerce/'.$src.'" class="pull-left" title="Click to view design">View</a>'
											.	'<a href="'.$base_url.'tshirtecommerce/download.php?key='.$design->rowid.'&view='.$view.'" class="pull-right" title="Click to view design">Download</a>'
											.'</div>';
								}
