var dgUI = {
	...
	category:{
		...
		add: function(id){
			UIModals.init(base_url+'index.php/clipart/editCategory');
		},
		remove: function(id){
			var remove = confirm(dgUI.category.lang.confirm_delete);
			if (remove == true)
			{
				jQuery.get( base_url+'index.php/clipart/deleteCategory/'+ id, function() {
		...
		edit: function(id){
			UIModals.init(base_url+'index.php/clipart/editCategory/'+id);
		},
		ini: function(){
		...
		tree: function(e, type){
			jQuery(e).dynatree({				
				...
				initAjax: {
				  url: base_url+"index.php/clipart/categoriestree/index.php"
				},
				...
				onClick: function (node, event) {
					...
					if (event.target.className == 'dynatree-title')
					{
						...
						jQuery.ajax({url: base_url + 'index.php/clipart/ajax/1/' + cateId}).done(function( data ) {
				...
				dnd: {
				  ...
				  onDrop: function(node, sourceNode, hitMode, ui, draggable) {
					...
					jQuery.post(base_url + "index.php/categories/sorting", { 'parent': node.data.id, 'child': sourceNode.data.id, 'action': hitMode},
					function(data) {});
	...
	art:{
		add: function(cateid){
			UIModals.init(base_url+'index.php/clipart/edit');			
		},
		...
		saveCategory: function(id)
		{
			...
			jQuery.ajax({					
				type: 'post',
				url: base_url+'index.php/clipart/saveCategory',
	...
	product:{
		...
		addCategoryJs: function(e, action){
			...
			if(action == 'save')
			{
				...
				var url = base_url + 'index.php/product/category';						
		...
		design: function(e, position){
			...
			jQuery.ajax({
				type: "POST",
				url: site_url + "index.php/product/design",
		...
		color:{
			...
			edit: function(id){
				...
				jQuery.ajax({
				type: "POST",
				url: base_url + "index.php/product/colors/dgUI.product.colorEdit/"+id				
				}).done(function( content ) {
		...
		fonts:{
			...
			save: function(e){
				...
				jQuery.ajax({
					type: 'POST',
					url: base_url + "admin/index.php/settings/addgooglefont",					
