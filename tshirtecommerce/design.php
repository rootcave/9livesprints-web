...
<html lang="en">
  ...
  <body>
									...
									else
									{
										...
										if($item['clipart_id']!=''){
											...
											if(isset($_GET['order_id']) && $_GET['order_id']!=''){
												$fethData = mysql_query("SELECT id FROM " . DB_PREFIX . "tshirtecommerce_clipart_use WHERE order_id='".$_GET['order_id']."'");

												$lp = "&order_id=".$_GET['order_id'];
	...
	function saveClipart(idval, ctitle, thumpath){
		var url = "../9livesprints_functions/clipart_save_counter.php?cid="+idval+"&ctitle="+ctitle+"&cthumb="+thumpath+"&ckey=<?php echo $_GET['key'].''.$lp; ?>";
