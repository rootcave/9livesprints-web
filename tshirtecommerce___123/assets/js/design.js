var url_ajax_product = 'index.php?route=tshirtecommerce/designer/ajax';
var design={
	zIndex: 1,
	design_id: null,
	design_file: '',
				...
				if (data != '')
				{
					if (typeof data.sale != 'undefined')
					{
						...
						if (data.sale == data.old)
							jQuery('#product-price-old').css('visibility', 'hidden');
						else
							jQuery('#product-price-old').css('visibility', 'visible');
					}
				}
			}).always(function(){
				lable.html(title);				
	...
	print:{
		...
		perpage: function(width, height){
			...
			for(i=1; i<=6; i++)
			{
				if (width < pagesW[i] && height<pagesH[i])
				{
					return 6 - i;
				}
			}
		...
		art:{
					...
					if (data.arts.length > 0)
					{
						...
						if (data.count > 1)
						{
							...
							var button = document.createElement('button');
								...
								button.innerHTML = 'Show More Designs';
							ul.html(button);
			...
			artDetail: function(e)
			{
				...
				if (document.getElementById('art-detail-'+id) == null)
				{
					...
					jQuery('.art-detail-price').html('From ' + currency_symbol + jQuery(e).data('price'));					
				}
				else
				{
					...
					if (typeof data.fonts.google_fonts != 'undefined')
					{
						...
						for(i=0; i<fonts.length; i++)
						{
							...
							if (i == count || fonts.length == i+1)
							{
								...
								jQuery('head').append("<link href='http://fonts.googleapis.com/css?family="+list_font+"' rel='stylesheet' type='text/css'>");
								list_font = '';
								j++;
								count = count * j;
							}
	...
	products:{
		...
		childCate: function(e){
			...
			jQuery.ajax({
				type: "POST",
				dataType: "json",
				url: baseURL + "index.php/products/getcategories",
				data: { id: e.value }
			}).done(function( data ) {
				if (data.status == 1)
				{
		...
		create: function(e){
			...
			jQuery.ajax({
				...
				var img = document.createElement('img');
					...
					img.setAttribute('title', 'Click to add QRcode');
